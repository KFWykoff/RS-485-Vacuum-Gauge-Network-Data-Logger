...
while True:
    ...
    def store_pressure(str):
        ...
        output_byte = ser.read(1)

        # continue reading single bytes until expected linefeed '*' is detected
        while output_byte != b'*':
            output_byte = ser.read(1)

        # if linefeed is detected read full return (13 characters)
        # retry in the case of value error
        #break loop only for successful pressure reading format
        while True:
            try:
        ...
        # return dt for later comparison to desired collection time (line )
        return(dt)
